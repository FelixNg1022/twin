...
## ADR-005: Hand-rolled turn dispatcher (not LangGraph's interrupt/checkpoint)

**Context.** The original design used LangGraph's `interrupt_before=[...]` + `MemorySaver` + `graph.update_state()` to pause the graph between user turns and resume on the next HTTP request. During live testing this broke hard: after `demographics → probe_weekend` transitioned with `demographics_pending_field=None`, the next HTTP turn's `update_state(config, {"messages": [...]})` reset all non-specified fields of `AgentState` back to their Pydantic defaults — including `demographics_pending_field` reverting from `None` to its default `"first_name"`. The conditional router (`_demographics_router`) re-evaluated on this stale state and routed back to `collect_demographics` instead of advancing to `probe_weekend`. The agent got stuck in an infinite name-asking loop.

Attempted fixes via `as_node=...` argument and via passing input to `astream(...)` directly either re-ran from the entry point or didn't correctly merge state.

**Decision.** Drop LangGraph's runtime machinery entirely for turn dispatch. The `build_graph()` function still exists and still compiles a `StateGraph` — that's used by `scripts/export_graph.py` to generate `docs/state-machine.md`'s mermaid diagram. But at request time, `app.agent.runner` holds sessions in an in-process `dict[str, AgentState]` and dispatches to node functions directly based on `state.current_node`. A tight loop runs nodes forward, halting when the current node is in a waiting set (`_WAITS_FOR_USER`) or when `complete=True`.

**Consequence.** No more state-reset footguns. Turn semantics are now trivially inspectable (one function, ~30 lines). We lose: LangGraph's cross-process persistence (sessions vanish on backend restart — acceptable for a demo), distributed-runner capability (not needed), and the "free" interrupt/resume UX (we replicate it manually in ~5 lines). We keep: the node topology as documentation, the mermaid visualization, all of the agent's behavioral code (nodes.py, prompts/). The video's architecture beat still works — the graph diagram is generated from the same `build_graph()` function even though runtime doesn't invoke it.

**Secondary fix packaged with this ADR:** `_pending_user_input()` rule — each waiting node now distinguishes fresh-entry (last message is assistant, don't re-process stale user input) from resume (last message is user, process it). This lets a node correctly handle both its first invocation (when the previous node's transition message already asked this node's question) and subsequent invocations (when the user has replied).
